...
// getClientIp lives in lib/clientIp.ts now, and is re-exported so every
// caller and this file's own suite are unchanged. It moved because the
// admin gate needs it and cannot import this module: middleware runs on the
// Edge runtime and @upstash/redis resolves to a Node build that reads
// process.version. Nothing in getClientIp was ever Node-specific - it reads
// request headers - it was only the company it kept.
