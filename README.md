...
| `VISITED_SHARE_RATE_LIMIT_PER_HOUR` / `_PER_DAY` | `120` / `600` | Per-IP cap on the anonymous visited-stats share write - the only write where an unauthenticated caller picks its own Redis key, held for 400 days. Generous because the visited page POSTs on every country toggle once a link exists |
| `SERVER_TOOL_COST_PER_1K_USD` | `10.0` | Price per 1,000 server-side tool requests (web search), used by the daily spend cap. These are billed per REQUEST, not in tokens, and were previously not counted at all - roughly a quarter of a generation's real cost |
